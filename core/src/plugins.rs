...
type FnWanStart = fn(
    *const c_void,
    *const c_void,
    extern "C" fn(*const c_void, *const u8, usize) -> i32,
) -> *const c_void;
type FnWanSend = fn(*const c_void, *const u8, usize);
type FnWanStop = fn(*const c_void) -> i32;
type FnWanDestroy = fn(*const c_void) -> i32;

pub type FnCallback = extern "C" fn(*const c_void, *const u8, usize) -> i32;
...
impl<'a> WanPlugin<'a> {
    ...
    pub fn start(
        ...
        let obj = (self.fn_start)(device.ptr, channel, cb);

        match obj.is_null() {
            true => Err(PluginError::SpawnFailed),
            false => Ok(WanInstance { obj, channel }),
        }
    }
